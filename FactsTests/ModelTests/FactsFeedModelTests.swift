...
class FactsFeedModelTests: XCTestCase {
    ...
    //MARK:- test model init
    func testModelInit() {
        
        let row = Row(title: "AAA", description: "This is unit testing", imageHref: "bbb")
        var rows = [Row]()
        rows.append(row)
        let model = FactsFeedModel(title: "Canada", rows: rows)
        
        //check whether title is correct
        XCTAssertEqual(model.title,"Canada")
        
        //check whether row title is correct
        XCTAssertEqual(model.rows[0].title,"AAA")
        
        //check whether description is correct
        XCTAssertEqual(model.rows[0].description,"This is unit testing")
        
        //check whether image href  is correct
        XCTAssertEqual(model.rows[0].imageHref,"bbb")
        
    }
    
    //MARK:- test whether JSON data is parsed populated correctly in the model
...
"""
        ...
        let result = try JSONDecoder().decode(FactsFeedModel.self, from:jsonData! )
        
        //check whether title is correct
        XCTAssertEqual(result.title,"About Canada")
        
        //check whether title of a row is correct
        XCTAssertEqual(result.rows[0].title, "Beavers")
        
        //check whether description is correct
        XCTAssertEqual(result.rows[0].description, "Beavers are second only to humans in their ability to manipulate and change their environment. They can measure up to 1.3 metres long. A group of beavers is called a colony")
        
        //check whether image link is correct
        XCTAssertEqual(result.rows[0].imageHref, "http://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/American_Beaver.jpg/220px-American_Beaver.jpg")
    }
